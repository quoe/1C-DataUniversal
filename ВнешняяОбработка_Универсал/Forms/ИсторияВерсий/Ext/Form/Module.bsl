...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВерсияОбработки") Тогда
		ВерсияОбработки = Параметры.ВерсияОбработки;
	КонецЕсли; 
	
	УКО_ФормыКлиентСервер_Заголовок(ЭтаФорма, СтрШаблон(НСтр("ru = 'История версий (ваша версия %1)'; en = 'Version history (your version %1)'"), ВерсияОбработки));
	
	Макет 	= ОбъектОбработки().ПолучитьМакет("ИсторияВерсий");
...
Процедура УКО_ПроверкаОбновленийКлиент_Инициализация(Форма) Экспорт
	...
	ЭлементДоступноОбновление = Форма.Элементы[УКО_ПроверкаОбновленийКлиентСервер_ИмяЭлементаКомандыДоступноОбновление()];
